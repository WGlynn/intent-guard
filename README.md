...
> **Fork notice.** This is a fork of [`uwecerron/intent-guard`](https://github.com/uwecerron/intent-guard) maintained for VibeSwap integration. The fork adds **8 production-shape adapters** (UUPS upgrades · treasury · LayerZero peers · AccessControl roles · Pausable · Ownable · bounded params · Merkle roots), **end-to-end integration tests for every adapter** wiring module + adapter + Safe + target, an **audit-readiness fix** for the module's legacy-compile-pipeline stack budget (PR upstreamed), and supporting docs (adapter authoring guide · CI workflow · security policy · contracts inventory · shared test helpers).
>
> **136 tests pass on the default Foundry profile** (no `--via-ir`). See [`FORK.md`](./FORK.md) for the inventory and [`docs/ADAPTERS.md`](./docs/ADAPTERS.md) for the authoring walkthrough.
>
